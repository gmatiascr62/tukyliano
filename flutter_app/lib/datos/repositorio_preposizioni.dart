...
/// Frases con hueco para practicar las preposiciones.
///
/// Mismo camino que los artículos y las frases: copia empaquetada con la app,
/// copia escribible en el celular, y chequeo contra GitHub por si hay frases
/// nuevas. Así se agregan sin publicar un APK.
class RepositorioPreposizioni {
  ...
  DatosPreposizioni _datos = DatosPreposizioni.vacio;
  bool _cargado = false;

  DatosPreposizioni get datos => _datos;
  ...
  Future<File?> _archivo() async {
    try {
      ...
      return dir == null
          ? null
          : File('${dir.path}/$archivoPreposizioniLocal');
    } catch (_) {
      return null;
    }
  }
  ...
  Future<void> cargar() async {
    ...
    try {
      ...
      final texto = archivo != null && await archivo.exists()
          ...
          : await _leerAsset(assetPreposizioni);
      _datos = DatosPreposizioni.desdeJson(
  ...
  Future<bool> verificarActualizacion() async {
    try {
      final respuesta = await _cliente
          .get(Uri.parse(urlPreposizioniRemoto))
      ...
      final nuevas = DatosPreposizioni.desdeJson(
          jsonDecode(texto) as Map<String, dynamic>);
